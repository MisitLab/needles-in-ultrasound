...
%
% The manual needle segmentations are used to define (and rotate) a region 
...
% to study the echogenicity of the needle tip and shaft relative to an 
% 'optimal' reflector, using a signal ratio.
%
...
% found in a moving average search function. 
%
...
% The FG sample width is equal to the needle diameter, and the length is:
...
% Needle types used in this study:                   <--------------------- Definition of used needle types. 
...
h.ndl_diam_px_default = round(2*h.pxmm); 
...
for idx = 1:vids
    ...
    % Define two frames for video resampling.
    h.frame(1,1) = round(h.ECs{idx,8}/5); % At 1/5th.
    if h.ECs{idx,6} > 68 % Correction for some vids with long idle time..
        h.frame(1,2) = round(3*h.ECs{idx,8}/4); % At 3/4th.
    else
        h.frame(1,2) = round(4*h.ECs{idx,8}/5); % At 4/5th.
    end
    for i = 1:2 % Retrieve frames as intensity maps (gray-scale).
        h.frameselection(:,:,i) = rgb2gray(read(...
            video,h.frame(1,i))); %#ok<VIDREAD>
    end
    % Note: if the needle 'idle time' is too long at the start or end of 
    % the movie and the needle is motionless in one of the manual segm. 
    % frames, the correct computation of intermediate needle-angles cannot 
    % be ascertained. The needle should be in motion in these frames!
end
% To do [for multiselect]: video focus based on selection in a dropdown 
% menu or listbox, for easy switching between video files.
h.v_focus = 1; 

% Check for existence of reference measurements.     <--------------------- Load data from reference surface measurements.
try refs=load('ref_collected_data','-mat','v');
    refs = refs.v; % Don't want the content nested.
    ln = length(fieldnames(refs))-1; % Minus the v.info field..
    for idx_v = 1:ln
        % Check if file name already exists in userdata.
        fieldnm = strcat('vid',num2str(idx_v)); % Fieldname.
        rd = refs.(fieldnm).samples; % Ref data.
        rd_array(idx_v,:) = reshape(rd,...
            [1,size(rd,1)*size(rd,2)*size(rd,3)]); %#ok<AGROW>
    end
    rd_tot = reshape(rd_array,[1,size(rd_array,1)*size(rd_array,2)]);
    h.ref_m = round(mean(double(rd_tot)));
    h.ref_s = round(std(double(rd_tot)));
catch % Allows code to run, even without ref measurements.
    h.ref_m = 255; % If no ref file available, assume max. whiteness.
    h.ref_s = 1; % Non-zero, but low variability in whiteness.
end
...
h.textSR_T = uicontrol(viewer,'Style','text','FontSize',18,'String',...
    'SR Tip:','BackgroundColor','white','HorizontalAlignment', 'left', ...
    'position', [x2_gui+40 100 x2_gui-100 30], 'ForegroundColor','blue', ...
    'Visible','off');
h.textSR_S = uicontrol(viewer,'Style','text','FontSize',18,'String',...
    'SR Shaft:','HorizontalAlignment', 'left','BackgroundColor',...
...
%% Callback functions (nested) for GUI interactions.
    ...
    function h = reset(h)
        ...
        set([h.texthelp,h.textangle,h.textSR_T,h.textSR_S,h.imshow_3],...
            'Visible', 'off');
        if isfield(h,'peak_x')
    ...
    function h = back_CB(~,~)
        ...
        counter = 0;
        if exist('ech_collected_data.mat','file')
    ...
    function h = update_axes(h)
        ...
        % Update text fields.
        round_SR_tip = num2str(round(100*h.(fit_name).SR_tip)/100);
        round_SR_sha = num2str(round(100*h.(fit_name).SR_sha)/100);
        set(h.textSR_T,'string',strcat('SR Tip:',round_SR_tip)); 
        set(h.textSR_S,'string',strcat('SR Shaft:',round_SR_sha)); 
        ...
        for fr = 1:length(h.angle_select)
            ...
            results.CNR_tip(fr) = h.(fit_name2).SR_tip;
            results.CNR_shaft(fr) = h.(fit_name2).SR_sha;
        end
        assignin('base','ECs',inputs)
        assignin('base','CNRs',results); 
        assignin('base','ROIs',{h.ROI,h.ROI_f});
        assignin('base','SMPLs',samples);
        % Note: Stored results are SR data. Code is kept the same as in the
        % visibility processing function (hence a common term CNR is used)
    ...
    % Outputs are the filtered ROI and a linear needle fit (cfit). This
    % code is a stripped version of the original: filtering is hardly
    % needed in the water measurements.
    function h = best_fit_fun(h,scan_frame) 
        ...
        %ROI2 = ROI; % Keeping a reference to original.
        h.range = 1:rows;                        %   <--------------------- Filter step 1: location-based image augmentation. 
        ...
        % Allowable deviation in the y-coord provided by black pixel rows.
        ROI3=ROI; % Keeping a reference.
        row_cuts = y_est+[-h.ndl_diam_px_default,h.ndl_diam_px_default];
        ROI3(1:row_cuts(1),:) = zeros(row_cuts(1),cols);
        ROI3(row_cuts(2):end,:) = zeros(rows-row_cuts(2)+1,cols);
        
        % Store to display.
        h.ROI_f(:,:,scan_frame) = uint8(ROI3); % Filtered image.
        ROI4_ind = double(reshape(ROI3,[1 rows*cols])); % Vector for search.
        ...
        n = 50; 
        [~,pl_f]=findpeaks(ROI4_ind,'SortStr','descend','MinPeakDistance',...
            round(rows/2),'NPeaks',n);
        [ROI_y_data, ~] = ind2sub(size(ROI),pl_f); 
    ...
    % a cfit line evaluation to find coordinates for the SR computations.
    ...
    % Note: all ROI samples are visualized as rectangles, whereas SR data 
    ...
    function h = local_tip_search(h,scan_frame)
        ...
        ndl_l_y = length(ndl_line_y);
        h.ndl_line_vals = zeros(h.ndl_d,ndl_l_y); 
        penalty = [fliplr(h.range)/max(h.range) ... 
            zeros(1,ndl_l_y-length(h.range))];
        for ndl_x = 1:ndl_l_y
        ...
        % tips are nonechogenic. This requires a conservative approach.
        % The new tip x-pos is allowed to shift max. with size h.tip_l. 
        window = h.tip_l; 
        if scan_frame == 1
    ...
    % Determine the foreground (FG) image samples and
    % calculate the signal ratio (SR). 
    function h = compute_SR(h,scan_frame)
        ...
        FG_tip = double(FG_tip); FG_sha = double(FG_sha); 
        FG_tip_m = mean(reshape(FG_tip,[1,size(FG_tip,1)*...
            size(FG_tip,2)]));
        FG_tip_s = std(reshape(FG_tip,[1,size(FG_tip,1)*...
            size(FG_tip,2)])); %#ok<NASGU>
        FG_sha_m = mean(reshape(FG_sha,[1,size(FG_sha,1)*...
            size(FG_sha,2)]));
        FG_sha_s = std(reshape(FG_sha,[1,size(FG_sha,1)*...
            size(FG_sha,2)])); %#ok<NASGU>
        % Note: std() did not like uint8 var type inputs.
                
        % Intensity ratio.                      %    <--------------------- Definition of SR values, computed for the needle tip and shaft.
        h.(fit_name).SR_tip = FG_tip_m/h.ref_m; 
        h.(fit_name).SR_sha = FG_sha_m/h.ref_m;
        % Note: the abbreviation is still termed SR, so that other data
        % visualization codes do not require additional if-then-loops.
    end
end
